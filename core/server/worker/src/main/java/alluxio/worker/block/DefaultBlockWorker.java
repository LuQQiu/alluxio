...
public final class DefaultBlockWorker extends AbstractWorker implements BlockWorker {
  ...
  public BlockReader getBlockReader(BlockReadRequest request) throws Exception {
    if (request.isPromote()) {
      try {
        moveBlock(request.getSessionId(), request.getId(), mStorageTierAssoc.getAlias(0));
    ...
    while (retryPolicy.attempt()) {
      ...
      try {
        if (openUfsBlock(request.getSessionId(), request.getId(),
          ...
          // TODO(lu) consider how to deal with the metrics
  ...
  @Override
  public void closeBlockReader(BlockReader reader, long sessionId, long blockId)
