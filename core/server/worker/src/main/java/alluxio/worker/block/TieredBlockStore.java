...
public class TieredBlockStore implements BlockStore {
  ...
  private TempBlockMeta createBlockMetaInternal(long sessionId, long blockId,
    ...
    try (LockResource r = new LockResource(mMetadataWriteLock)) {
      ...
      StorageDir dir = mAllocator.allocateBlockWithInfo(sessionId, initialBlockSize, location,
          Collections.unmodifiableList(mMetaManager.getTiers()),
          Collections.unmodifiableMap(mMetaManager.getAliasToTiersMap()));
      if (dir == null) {
