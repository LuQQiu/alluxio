...
public class RaftJournalWriter implements JournalWriter {
  ...
  private final long mShouldFlushBytes;
  ...
  public RaftJournalWriter(long nextSequenceNumberToWrite,
    ...
    mShouldFlushBytes =
        ServerConfiguration.getBytes(PropertyKey.MASTER_EMBEDDED_JOURNAL_SHOULD_FLUSH_BYTES);
  }
  ...
  public void write(JournalEntry entry) throws IOException, JournalClosedException {
    ...
    if (mCurrentJournalEntrySize.addAndGet(entry.getSerializedSize()) > mShouldFlushBytes) {
      flush();
    }
  }
